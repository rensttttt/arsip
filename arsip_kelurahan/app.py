...
def dashboard():
    ...
    try:
        ...
        if not conn or not conn.is_connected():
            ...
            logging.error(f"[{error_id}] Dashboard: Database connection failed")
            log_system_error(
                module="Dashboard",
        ...
        logging.error(f"[{error_id}] Dashboard MySQL error: {e}", exc_info=True)
        log_system_error(
            module="Dashboard",
        ...
        logging.error(f"[{error_id}] Dashboard unexpected error: {e}", exc_info=True)
        log_system_error(
            module="Dashboard",
...
def activity(action_filter=None):
    ...
    try:
        ...
        for activity in activities:
            ...
            # Format details menjadi deskripsi yang ramah pengguna
            if activity.get('details'):
                try:
                    details = json.loads(activity['details'])
                    formatted_details = format_activity_details(details, activity['action'])
                    activity['details'] = formatted_details
                except json.JSONDecodeError:
                    activity['details'] = 'Detail tidak valid'
            else:
...
def format_activity_details(details, action):
    """
    Format detail JSON menjadi deskripsi yang ramah pengguna berdasarkan jenis aktivitas.
    Menghindari menampilkan data sensitif seperti query SQL.
    """
    try:
        if action == 'view_activity':
            page = details.get('page', 'tidak diketahui')
            total_activities = details.get('total_activities', 0)
            action_filter = details.get('action_filter', 'semua')
            return f"Membuka halaman {page} dengan filter '{action_filter}', total aktivitas: {total_activities}"
        elif action == 'dashboard':
            route = details.get('route', 'tidak diketahui')
            method = details.get('method', 'tidak diketahui')
            return f"Mengakses dashboard melalui rute {route} dengan metode {method}"
        elif action in ['upload', 'download', 'delete', 'view']:
            archive_id = details.get('archive_id', 'tidak diketahui')
            return f"Aksi {action} pada arsip dengan ID {archive_id}"
        elif action == 'profile_update':
            return "Memperbarui profil pengguna"
        elif action == 'password_change':
            return "Mengubah kata sandi pengguna"
        else:
            # Hindari menampilkan detail sensitif seperti query SQL
            safe_details = {k: v for k, v in details.items() if k not in ['query', 'sql', 'csrf_token']}
            return ", ".join([f"{key}: {value}" for key, value in safe_details.items()]) or "Tidak ada detail tambahan"
    except Exception as e:
        logging.warning(f"Error formatting activity details: {e}")
...
def api_profile():
    ...
    try:
        ...
        # Fetch user data
        cursor.execute("""
            SELECT id, username, email, full_name, phone, is_admin, is_superadmin, is_active,
                   email_verified, last_login, created_at, avatar_url, address, position, department
        ...
        # Fetch stats
        cursor.execute("SELECT COUNT(*) as count FROM archive WHERE user_id = %s", (current_user.id,))
        ...
        cursor.execute("SELECT COUNT(*) as count FROM activity_log WHERE user_id = %s AND created_at >= %s",
                      (current_user.id, datetime.now(pytz.timezone('Asia/Jakarta')).replace(day=1)))
...
def api_profile_avatar():
    ...
    if file:
        try:
            ...
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE user SET avatar_url = %s WHERE id = %s", (f"/{file_path}", current_user.id))
            conn.commit()
            ...
            return jsonify({'message': 'Avatar berhasil diperbarui'})
...
def archive_list():
    ...
    Args:
        ...
        category_id (int): Filter by category ID from categories table
    ...
    try:
        ...
        if not conn or not conn.is_connected():
            ...
            log_system_error(
                ...
                ip_address=request.remote_addr or 'unknown',
                user_id=current_user.id,
                details={'route': request.path, 'sort': sort, 'query': query, 'category_id': category_id}
            )
        ...
        count_query = f"""
            ...
            FROM archives 
            JOIN archive_category ON archives.id = archive_category.archive_id
            {where_clause}
        """
        ...
        if page > total_pages:
            ...
            return redirect(url_for('archive_list', page=total_pages, sort=sort, query=query, category_id=category_id, date_range=date_range))

        # Fetch archives with category name and description
        query = f"""
            SELECT 
                archives.id, 
                archives.title, 
            ...
            FROM archives
            JOIN archive_category ON archives.id = archive_category.archive_id
            JOIN categories ON archive_category.category_id = categories.id
            {where_clause}
            ORDER BY {order_by}
            LIMIT %s OFFSET %s
        """
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                log_activity(
                    ...
                    ip_address=request.remote_addr or 'unknown',
                    user_agent=request.user_agent.string or 'unknown',
                    description=f"Pengguna {current_user.username} mengunggah arsip '{title}' dengan kategori ID {category_id}",
                    details={'archive_id': archive_id, 'filename': filename, 'category_id': category_id}
                )
